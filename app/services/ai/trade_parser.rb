module Ai
  ...
  class TradeParser
    SYSTEM = <<~SYS.freeze
      ...
      Cấu trúc:
      {"side":"buy"|"sell","symbol":"MÃ_VIẾT_HOA","price":<giá VND mỗi cổ phiếu, số nguyên>,"quantity":<số cổ phiếu hoặc null>}
    ...
    Result = Struct.new(:ok, :side, :symbol, :price, :quantity, :error, keyword_init: true)
    ...
    def self.parse(text, user:)
      ...
      Result.new(ok: true, side: side, symbol: symbol, price: price, quantity: qty)
