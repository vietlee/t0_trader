...
Rails.application.configure do
  ...
  # Redis cache store (dùng DB riêng để không đụng VOX trên cùng máy).
  config.cache_store = :redis_cache_store, { url: ENV.fetch("REDIS_CACHE_URL", "redis://localhost:6379/2") }

  # Sidekiq cho background jobs (AI analysis).
  ...
  # Host allowlist — chống DNS rebinding.
  config.hosts = [
    ENV.fetch("APP_HOST", "stock.czin.net"),
    /.*\.czin\.net/
  ]
  # Bỏ qua kiểm tra host cho health check.
